...
def create_metadata_sheet(f,export_name,vs30,rlz_lt,gsim_lt_dict):
    ...
    summary.loc['analysis_record','col'] = export_name
...
def create_metadata_file(export_name,vs30,rlz_lt,gsim_lt_dict):

    if not os.path.exists(export_name):
        os.makedirs(export_name)

    filename = Path(export_name,f'{export_name}_metadata.xlsx')
    with pd.ExcelWriter(filename) as f:
        create_metadata_sheet(f,export_name,vs30,rlz_lt,gsim_lt_dict)
...
def create_hcurves_single_vs30(export_name,vs30,parameters_for_exporting,data):
    
    sites_for_exporting = parameters_for_exporting['sites']
    imts_for_exporting = parameters_for_exporting['imts']
    
    metadata_file = Path(export_name,f'{export_name}_metadata.xlsx')
    metadata = pd.read_excel(metadata_file,sheet_name='metadata')
    
    filename = Path(export_name,f'{export_name}_hcurves_vs30-{vs30}.xlsx')
    with pd.ExcelWriter(filename) as f:

        metadata.to_excel(f,sheet_name='metadata',header=False,index=False)
        _ = [f.sheets['metadata'].set_column(i,i,max([len(str(s)) for s in metadata[col].values]+[len(col)])+2) for i,col in enumerate(metadata.columns)]
...
def create_uhs_single_vs30(export_name,vs30,parameters_for_exporting,data):
    ...
    filename = Path(export_name,f'{export_name}_uhs_vs30-{vs30}.xlsx')
    with pd.ExcelWriter(filename) as f:
