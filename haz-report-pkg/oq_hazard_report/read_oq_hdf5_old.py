...
def retrieve_data(file_id, data_dir, named_sites=True):
    '''
    ...
    # dstore = datastore.read(file_id,datadir=data_dir)
    dstore = datastore.read(file_id)
    oqparam = vars(dstore['oqparam'])
    data['metadata'] = {}
    data['metadata']['imtls'] = oqparam['hazard_imtls']
    if named_sites:
        data['metadata']['sites'] = find_site_names(dstore.read_df('sitecol'))
    else:
        data['metadata']['sites'] = dstore.read_df('sitecol')

    dstore.close()

    data['hcurves'] = {}
    with h5py.File(file_id, 'r') as hf:
        data['hcurves']['hcurves_rlzs'] = np.moveaxis(hf['hcurves-rlzs'][:], 1, 3)
        data['hcurves']['hcurves_stats'] = np.moveaxis(hf['hcurves-stats'][:], 1, 3)
...
def find_site_names(sites,dtol=0.001):
    ...
    with open('/home/chrisdc/NSHM/DEV/nzshm_hazlab/haz-report-pkg/oq_hazard_report/resources/locations.json','r') as f:
