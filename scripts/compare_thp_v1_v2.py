...
fig_dir = Path("/home/chrisdc/NSHM/oqresults/THP_v1_v2/figs")
...
for location, coded_location in zip(locations, coded_locations):
    ...
    for imt in imts:
        ...
        for k, agg in enumerate(aggs):
            ...
            diff_max = max(diff_max, max(diff))
            diff_rel_max = max(diff_rel_max, max(rel_diff))


        print(f"max diff:  {diff_max:.2e}")
        print(f"max rel diff: {diff_rel_max:.2e}")
        print("")

        hcurve_filepath = fig_dir / f"v1v2-hcurve-{location}-{imt}.png"
        diff_filepath = fig_dir / f"v1v2-diff-{location}-{imt}.png"
        reldiff_filepath = fig_dir / f"v1v2-reldiff-{location}-{imt}.png"
